...
class ActivityManager(object):
	...
	def goto(self,coords):
		...
		#set goal
    		...
    		goal.target_pose.pose.orientation.z = math.sin(math.radians(float(coords['theta']))/2)
    		goal.target_pose.pose.orientation.w = math.cos(math.radians(float(coords['theta']))/2)
		...
		print "Going to %s, %s, %s" % (coords['x'],coords['y'], coords['theta'])

	def say(self,action):
		message = action['message']
		print "Saying %s" % message
		subprocess.call(["spd-say","-r","-50",message])
		time.sleep(2)
		"""self.top = Tk()
		...
		f.pack()
		B = Button(f, text ="OK", command = self.destroyWindow)
		B['font'] = helv36
		B.pack()
		self.top.mainloop()"""
